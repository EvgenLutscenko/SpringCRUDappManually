...
function edit_task(task_id) {
    ...
    $(identifier_edit).attr("id", "save_" + task_id);

    let identifier_save = "#save_" + task_id;
    $(identifier_save).html("Save");

    let property_save_tag = "update_task(" + task_id + ")";
    $(identifier_save).attr("onclick", property_save_tag);

    let current_tr_element = $(identifier_save).parent().parent();
